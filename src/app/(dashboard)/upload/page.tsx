...
export default function UploadPage() {
  ...
  const handleUploadComplete = async (res: { url: string; name: string; size: number }[]) => {
    ...
    try {
      await axios.post('/api/upload/process', {
        url: file.url,
        name: file.name,
        size: file.size,
      });
      ...
      <div className='grid gap-6 md:grid-cols-2'>
        <Card className='flex flex-col'>
          ...
          <CardContent className='flex-1 flex flex-col justify-center'>
            <UploadDropzone<OurFileRouter, 'excelUploader'>
              endpoint='excelUploader'
              onClientUploadComplete={handleUploadComplete}
              onUploadError={(error: Error) => {
                toast.error(`Upload failed: ${error.message}`);
              }}
              className='ut-label:text-primary ut-button:bg-primary border-dashed border-2 hover:bg-muted transition-colors rounded-xl p-8'
            />
          </CardContent>
        </Card>
